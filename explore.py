...
def standard_scaler(train, validate, test):
    ...
    train = train.select_dtypes(exclude=['category', 'object'])
    validate = validate.select_dtypes(exclude=['category', 'object'])
    test = test.select_dtypes(exclude=['category', 'object'])
    ...
    validate_scaled = pd.DataFrame(scaler.transform(validate),
                                   columns=validate.columns.values).set_index([validate.index.values])
    test_scaled = pd.DataFrame(scaler.transform(test), 
                               columns=test.columns.values).set_index([test.index.values])
